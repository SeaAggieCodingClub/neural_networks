...
class PacGame:
    ...
    def __init__(self):
        ...
        self.pacman_speed = 8.0 / self.fps # Tiles per second / Frames per second = Tiles per frame
        self.ghosts_speed = [
                self.pacman_speed * 0.80, # 80 % - Level 1
                self.pacman_speed * .90,  # 90 % - Levels 2-4
                self.pacman_speed         # 100% - Levels 5+
            ]
        
       
        self.pacman = Pacman.Pacman(self.pacman_speed) # CHANGE FROM GHOST CLASS TO PACMAN CLASS
        self.pacman.pos = Position(21, 26)
        self.ghosts = [
        Ghosts.Ghost('r', self.ghosts_speed[0], ""),
        Ghosts.Ghost('p', self.ghosts_speed[0], ""),
        Ghosts.Ghost('b', self.ghosts_speed[0], ""),
        Ghosts.Ghost('o', self.ghosts_speed[0], "")
        ]
        # Initialize other variables
        self.pacman.dir = 'a' # Initial pacman dirdw
    
        self.pellets = self.get_pellets(grid)
        self.phase = 's' # Begin in scatter mode
        self.seconds = 0
        self.phase_rotation = 0
        self.level = 1
        ...
        self.pac = pygame.image.load("Pacman/images/pacman.png")
        self.pacu = pygame.transform.scale(self.pac, (45,45))

        self.targets = []
        for _ in range(4):
            self.img = pygame.image.load("Pacman/images/target.png") # Load the image
            self.targets += [pygame.transform.scale(self.img, (35, 35))] # Scale the image and reference it back to the key

        
        for ghost in self.ghosts:
            ghost.target = self.pacman.pos # Update target
        ...
        # Menu variables
        self.outside_border = pygame.Rect(10, 15, 200, height - 30)
        self.inside_border = pygame.Rect(15, 20, 190, height - 40)
...
#change this to a class with a main loop function
    ...
    def loop(self):
        ...
        self.pellets = self.get_pellets(grid)
        # Phase timer
        if self.phase != 'f': # Pause timer if in frightened mode
            self.seconds += 1 / self.fps # Increment timer
        #print(int(seconds))
        prev_phase = self.phase # Hold variable
        if self.phase_rotation <= 4: # Only for 4 rotations 
            if self.level == 1:
                if self.seconds > self.phase_lengths[0][self.phase][self.phase_rotation]:
                    self.phase, self.phase_rotation = self.phase_switch(self.phase, self.phase_rotation)
            elif self.level < 4:
                if self.seconds > self.phase_lengths[1][self.phase][self.phase_rotation]:
                    self.phase, self.phase_rotation = self.phase_switch(self.phase, self.phase_rotation)
            else: # Levels 5+
                if self.seconds > self.phase_lengths[2][self.phase][self.phase_rotation]:
                    self.phase, self.phase_rotation = self.phase_switch(self.phase, self.phase_rotation)
            if self.phase != prev_phase: # If the phase has changed
                self.seconds = 0 # Reset seconds
                Ghosts.switch_phase(self.ghosts, self.phase, prev_phase) # Update ghosts
        
        # Update ghost data
        Ghosts.update_ghosts(self.ghosts, self.pacman, grid, self.phase, self.fps, self.seconds, self.phase_rotation, self.pellets)
    
        # Update display
        for ghost in self.ghosts:
            self.display(Ghosts.images["body"][ghost.id], ghost.pos)
            #display(images["eyes"], ghost.pos)
        
        match self.pacman.dir:
            case 'w':
                self.display(pygame.transform.rotate(self.pacu,90), self.pacman.pos)
            case 'a':
                self.display(pygame.transform.rotate(self.pacu,180), self.pacman.pos)
                
            case 's':
                self.display(pygame.transform.rotate(self.pacu,270), self.pacman.pos)
            case 'd':
                self.display(self.pacu,self.pacman.pos)
        ...
        # Cap the frame rate
        self.clock.tick(self.fps)  # Adjust as necessary for smoothness
        ...
        # Pacman Eating Dots
        if grid[27 - round(self.pacman.pos.x),round(self.pacman.pos.y)] == 'dot_':
            grid[27 -round(self.pacman.pos.x),round(self.pacman.pos.y)] = '____'
            Sound.play_waka(True)
        
        # Update pacman direction
        Pacman.move(self.pacman, grid)
        #pacman.move(pacman.speed)
        Ghosts.check_warp_tunnels(Ghosts.warp_tunnels, self.pacman)
...
def train_ai(genome, config):
    ...
    while run:
        ...
        output = net.activate(
                (game.pellets, game.pacman.pos.x, game.pacman.pos.y))
        ...
        if score != game.pellets:
            sec = game.seconds
            print("gained!" + str(sec))
            
        if score == game.pellets and int(sec+5) == int(game.seconds):
            genome.fitness = game.pellets
            break
