...
class PacManGame(gym.Env):
    ...
    def flatten_state(self):
        ...
        def extract_values(value):
            ...
            if isinstance(value, tuple): # Process dictionary values
                for v in value:
                    flat_list.append(v)
            elif isinstance(value, dict): # Process dictionary values
                for v in value.values():
                    extract_values(v)
            elif isinstance(value, np.ndarray): # Process array
                for pos in value.flat:
                    extract_values(pos)
            else:
                flat_list.append(value)
    ...
    def get_pellets_pos(self):
        '''Returns a numpy array of each pellet position'''
        pellets_pos = []
        for loc, val in self.grid.items(): # For each value
    ...
    def update_pellets(self, reward):
        ...
        if 0 <= pos.x <= 27: # Check if indices are in range
            ...
            if grid_value == 'pdot':
                ...
                pacman.score += 40 # Total of 50 points
    ...
    def step(self, action=None, prev_action=None, prev_pos=None):
        ...
        # End game check
        if pacman.lives == 0 or self.level == 255:
            done = True
